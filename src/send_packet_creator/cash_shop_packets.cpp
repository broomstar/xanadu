...
void
PacketCreator::GetCashShopInventory(std::vector<std::shared_ptr<Item>> items,
...
void PacketCreator::ShowBoughtCashItem(const std::shared_ptr<Item> &item,
                                       int account_id) {
    write<short>(send_headers::kCASHSHOP_OPERATION);
...
void PacketCreator::IncreaseInventorySlots(signed char inventory_id,
                                           signed char slots) {
    write<short>(send_headers::kCASHSHOP_OPERATION);
...
void
PacketCreator::TransferToCashShopInventory(const std::shared_ptr<Item> &item,
                                           int account_id) {
    write<short>(send_headers::kCASHSHOP_OPERATION);
...
void PacketCreator::CashShopShowBoughtPackage(
        std::vector<std::shared_ptr<Item>> items, int account_id) {
    write<short>(send_headers::kCASHSHOP_OPERATION);
...
void PacketCreator::CashShopAddCashItemData(const std::shared_ptr<Item> &item,
                                            int account_id) {
    ...
    write_string("",
                 13); // character name? (who made the gift?) sBuyCharacterID db 13 dup(?)
    write<long long>(
            item->get_expiration_time()); // dateExpire      _FILETIME ?
