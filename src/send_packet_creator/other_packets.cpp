...
void PacketCreator::GainItem(int itemid, short amount) {
    ...
    write<signed char>(
            0); // mode: 0 = drop pickup, 1 = quest message, 3 = increase exp, 4 = increase fame, 5 = increase mesos, there are also much other types
    write<signed char>(
            0); // mode2 for drop pickup: -1 = can't pickup, 0 = item, 1 = mesos
    write<int>(itemid);
    write<int>(amount);
}
...
void PacketCreator::GainMesos(int amount) {
    ...
    write<signed char>(
            0); // mode: 0 = drop pickup, 1 = quest message, 3 = increase exp, 4 = increase fame, 5 = increase mesos, there are also much other types
    write<signed char>(
            1); // mode2 for drop pickup: -1 = can't pickup, 0 = item, 1 = mesos
    write<signed char>(0);
    write<int>(amount);
    write<short>(0);
}
...
void PacketCreator::CantGetAnymoreItems() {
    ...
    write<signed char>(
            0); // mode: 0 = drop pickup, 1 = quest message, 3 = increase exp, 4 = increase fame, 5 = increase mesos, there are also much other types
    write<unsigned char>(
            0xFF); // mode2 for drop pickup: -1 = can't pickup, 0 = item, 1 = mesos
}

void PacketCreator::UpdateQuestInfo(signed char mode, Quest *quest) {
    write<short>(send_headers::kSHOW_STATUS_INFO);
    write<signed char>(
            1); // mode: 0 = drop pickup, 1 = quest message, 3 = increase exp, 4 = increase fame, 5 = increase mesos, there are also much other types
...
void
PacketCreator::GainExp(int exp, bool in_chat, bool white, int party_bonus) {
    write<short>(send_headers::kSHOW_STATUS_INFO);
    write<signed char>(
    ...
    write<signed char>(
            0); // 0 = enable Bonus EXP for PARTY (+value), 1 = enable Bonus Event Party EXP (+value) x0
...
void PacketCreator::FameGainChat(int amount) {
    write<short>(send_headers::kSHOW_STATUS_INFO);
    write<signed char>(
            4); // mode: 0 = drop pickup, 1 = quest message, 3 = increase exp, 4 = increase fame, 5 = increase mesos, there are also much other types
    write<int>(amount);
}
...
void PacketCreator::MesosGainChat(int amount) {
    write<short>(send_headers::kSHOW_STATUS_INFO);
    write<signed char>(
            5); // mode: 0 = drop pickup, 1 = quest message, 3 = increase exp, 4 = increase fame, 5 = increase mesos, there are also much other types
    write<int>(amount);
}

void
PacketCreator::ShowAvatarMega(Player *player, unsigned char ear, int item_id,
...
void PacketCreator::ShowPlayer(Player *player) {
    ...
    if (has_minigame) {
        ...
        write<signed char>(
                1); // amount of players that are already inside the minigame
    ...
    write<signed char>(
            0); // berserk skill darkforce effect - 1 or 0 probably (skillid: 1320006, job: dark knight - which is an explorer and warrior)
    write<signed char>(0);
...
void
PacketCreator::ShowBossHp(int mob_id, int hp, int max_hp, signed char color,
                          signed char background_color) {
    write<short>(send_headers::kMAP_EFFECT);
    write<signed char>(
            5); // mode that specifies the type of effect: 2 = object, 3 = effect, 4 = sound, 5 = boss hp, 6 = map music
    write<int>(mob_id);
    write<int>(hp);
    write<int>(max_hp);
...
void
PacketCreator::PlayerAttack(signed char attack_type, PlayerAttackInfo &attack) {
    switch (attack_type) {
...
void PacketCreator::AddInventoryInfo(Player *player) {
    ...
    auto items = player->get_inventory(
            kInventoryConstantsTypesEquipped)->get_items();
...
void PacketCreator::AddCharLook(Player *player, bool megaphone) {
    ...
    auto equips = player->get_inventory(
            kInventoryConstantsTypesEquipped)->get_items();
...
void PacketCreator::ShowPlayerMovement(int player_id, short start_position_x,
...
void PacketCreator::ShowInfo(Player *player) {
    ...
    if (!inventory) {
        ...
        auto medal = inventory->get_item_by_slot(
                kItemConstantsEquippedSlotsMedal);
...
void
PacketCreator::ItemGainChat(int itemid, int amount, signed char items_size) {
    write<short>(send_headers::kSHOW_ITEM_GAIN_INCHAT);
    write<signed char>(
            3); // actions: 3 = gain item, 4 = pet level up, there are also much other types
...
void PacketCreator::ShowOwnPetLevelUp(signed char pet_slot) {
    write<short>(send_headers::kSHOW_ITEM_GAIN_INCHAT);
    write<signed char>(
            4); // actions: 3 = gain item, 4 = pet level up, there are also much other types
    write<signed char>(0);
...
void PacketCreator::ShowForeignEffect(int player_id, signed char effect) {
    ...
    write<int>(player_id);
    write<signed char>(
            effect); // 0 = level up, 4 = pet level up (has custom structure), 8 = job change, others are (partially) buffs/skills?
}
...
void PacketCreator::ShowPetLevelUp(int owner_player_id, signed char pet_slot) {
    ...
    write<signed char>(
            4); // 0 = level up, 4 = pet level up (has custom structure), 8 = job change, others are (partially) buffs/skills?
    write<signed char>(0);
    write<signed char>(pet_slot);
}

void PacketCreator::ShowBuffEffect(int player_id, signed char effect_id,
                                   int skill_id, signed char skill_level) {
    if (player_id > 0) {
        write<short>(send_headers::kSHOW_FOREIGN_EFFECT);
        write<int>(player_id);
    } else {
    ...
    write<signed char>(
            effect_id); // actions for kSHOW_FOREIGN_EFFECT: 0 = level up, 8 = job change, others are (partially) buffs/skills?
    write<int>(skill_id);
