...
void PacketCreator::ShowNpcShop(ShopData *data) {
    ...
    for (auto it : *items) {
        ...
        write<int>(
                0); // perfect pitch: amount of it needed for purchase (works properly only if 0 is writen for item price)
        write<int>(
                0); // can be used for <value> minutes after purchase (for USE items and maybe more)
        write<int>(0);
...
void PacketCreator::send_style(int styles[], int size, int npc_id,
                               std::string &text) {
    write<short>(send_headers::kNPC_TALK);
    write<signed char>(4);
    write<int>(npc_id);
...
void PacketCreator::GetFredrickStorage(
...
void PacketCreator::GetStorage(int npc_id, signed char slots,
...
void PacketCreator::StoreOrTakeOutStorage(bool store, signed char slots,
                                          signed char inventory_id,
                                          std::vector<std::shared_ptr<Item>> items) {
    write<short>(send_headers::kOPEN_STORAGE);
    write<signed char>(store ? storage_packet_action_constants::kStoreItems
                             : storage_packet_action_constants::kTakeOut);
    write<signed char>(slots);
