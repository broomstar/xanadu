...
void
PacketCreator::ShowTrade(Player *player, Player *partner, signed char number) {
...
void PacketCreator::GetHiredMerchant(Player *player,
                                     std::shared_ptr<HiredMerchant> merchant,
                                     bool first_time) {
    write<short>(send_headers::kPLAYER_INTERACTION);
    write<signed char>(5); // action
    write<signed char>(5);
    write<signed char>(4);
    write<signed char>(
            merchant->is_owner(player->get_id()) ? 0 : merchant->get_playerslot(
                    player->get_id()));
    write<signed char>(0);
    ...
    for (size_t i = 0; i < merchant->get_num_items(); ++i) {
        std::shared_ptr<HiredMerchantItem> sellItem = merchant->get_item(
                static_cast<short>(i));
...
void PacketCreator::UpdateHiredMerchant(HiredMerchant *hired_merchant) {
    ...
    write<signed char>(
            static_cast<unsigned char>(hired_merchant->get_num_items()));

    for (int i = 0; i < hired_merchant->get_num_items(); i++) {
        std::shared_ptr<HiredMerchantItem> sellItem = hired_merchant->get_item(
                static_cast<short>(i));
...
void
PacketCreator::SpawnHiredMerchant(std::shared_ptr<HiredMerchant> merchant) {
