...
#include "constants/guild_constants.hpp"
#include "player.hpp"
...
void PacketCreator::GetGuildInfo(Guild *guild) {
    ...
    write_string(guild->GetRank1());
    write_string(guild->GetRank2());
    write_string(guild->GetRank3());
    write_string(guild->GetRank4());
    write_string(guild->GetRank5());
    {
        // guild member data
...
void PacketCreator::GuildPlayerLeave(int guild_id, int player_id,
                                     std::string charname, bool expelled) {
    write<short>(send_headers::kGUILD_OPERATION);
    write<signed char>(expelled ? GuildSendPacketActions::kPlayerExpelled
                                : GuildSendPacketActions::kPlayerLeave);
    write<int>(guild_id);
    write<int>(player_id);
    write_string(charname);
}
...
void
PacketCreator::GuildMemberOnline(int guild_id, int player_id, bool online) {
    write<short>(send_headers::kGUILD_OPERATION);
...
void PacketCreator::guild_bbs_thread_list(int start) {
    ...
    if (!has_entries) {
        write_bool(false); // has no notice
...
void PacketCreator::guild_bbs_show_thread(int local_thread_id) {
    ...
    for (i = 0; i < reply_count/* && repliesRS.next()*/; i++/*++i*/) {
        ...
        write<int>(0); // poster cid
        write<long long>(
                0); // timestamp // mplew.writeLong(getKoreanTimestamp(repliesRS.getLong("timestamp")));
...
void
PacketCreator::MakeNewAlliance(/*MapleAlliance alliance, */Player *player) {
    write<short>(send_headers::kGUILD_ALLIANCE_OPERATION);
...
void
PacketCreator::GetGuildAlliances(/*MapleAlliance alliance, */Player *player) {
    write<short>(send_headers::kGUILD_ALLIANCE_OPERATION);
...
void
PacketCreator::AddGuildToAlliance(/*MapleAlliance alliance, */int new_guild,
                                                              Player *player) {
    write<short>(send_headers::kGUILD_ALLIANCE_OPERATION);
...
void PacketCreator::GuildAllianceMemberOnline(Player *player, bool online) {
    write<short>(send_headers::kGUILD_ALLIANCE_OPERATION);
    write<signed char>(
            GuildAllianceSendPacketActions::kAllianceGuildMemberOnline);
...
void PacketCreator::ChangeAllianceRankTitle(int alliance,
                                            std::vector<std::string> ranks) {
    write<short>(send_headers::kGUILD_ALLIANCE_OPERATION);
    write<signed char>(
            GuildAllianceSendPacketActions::kChangeAllianceRankTitle);
    write<int>(alliance);
    for (auto it : ranks) {
        write_string(it);
    }
...
void PacketCreator::RemoveGuildFromAlliance(/*MapleAlliance alliance, */
        int expelled_guild, Player *player) {
    write<short>(send_headers::kGUILD_ALLIANCE_OPERATION);
    write<signed char>(
            GuildAllianceSendPacketActions::kRemoveGuildFromAlliance);
