...
void
PacketCreator::get_handshake(unsigned char *iv_recv, unsigned char *iv_send) {
...
*/
void
PacketCreator::LoginRequest(signed char success_or_failure_reason, int user_id,
    ...
    write<int>(user_id);
    write<signed char>(
            kGenderConstantsMale); // gender byte 0 = male, 1 = female, number 10/0x0A triggers gender select
    write<signed char>(
            0); // nGradeCode (admin byte: bool, true/1 for GMs/admins, false/0 for normal), for GM commands and maybe other stuff
    /*
    ...
    */
    ...
    write<signed char>(1);
    write<signed char>(
            1); // PIC modes: 0 = Register PIC | 1 = Ask for PIC | 2 = Disable PIC
}
...
*/
void PacketCreator::GetViewAllCharactersLoginError(
        signed char success_or_failure_reason) {
...
void PacketCreator::ShowAllCharacterInfo(int world_id,
...
void PacketCreator::EndWorlds() {
    write<short>(send_headers_login::kSERVER_LIST);
    write<signed char>(
            kServerListPacketNoWorlds); // kServerListPacketNoWorlds = no worlds, >= 0 is world id with data
}
...
// 7 = ID already logged in or server under inspection popup
void
PacketCreator::ShowCharacters(std::unordered_map<int, Character *> *characters,
                              int character_slots) {
