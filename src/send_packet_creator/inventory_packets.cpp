...
void PacketCreator::ItemInfo(Item *item, bool show_position) {
    ...
    signed char item_type = (item->is_pet() ? kPet : item->is_equip() ? kEquip
                                                                      : kItem);
    ...
    switch (item_type) {
        case kEquip: {
            ...
            write<signed char>(
                    0); // item level (usually starts with 120) - only has relevance for timeless equips
            write<int>(
                    0); // item exp (like this ?: 0 * 100000; // 10000000 = 100% ? - apparently not entirely correct) - only has relevance for timeless equips
...
void PacketCreator::UpdateSlot(std::shared_ptr<Item> item) {
    ...
    write<signed char>(
            1); // mode (0 = add item, 1 = update quantity, 2 = move, 3 = remove)
...
void PacketCreator::MoveItem(signed char inventory_id, short source_slot,
    ...
    write<signed char>(
            2); // mode (0 = add item, 1 = update quantity, 2 = move, 3 = remove)
...
void PacketCreator::NewItem(std::shared_ptr<Item> item, bool from_drop) {
    ...
    write<signed char>(
            0); // mode (0 = add item, 1 = update quantity, 2 = move, 3 = remove)
    write<signed char>(item->get_inventory_id());
    write<short>(item->get_slot());
...
PacketCreator::remove_item(signed char inventory_id, int slot, bool from_drop) {
    ...
    write<signed char>(
            3); // mode (0 = add item, 1 = update quantity, 2 = move, 3 = remove)
    write<signed char>(inventory_id);
    write<short>(slot);
...
void
PacketCreator::MoveItemMergeTwo(signed char inventory_id, short source_slot,
...
void PacketCreator::ScrolledItem(std::shared_ptr<Item> scroll,
    ...
    if (scroll->get_amount() > 1 && scroll->get_slot() < 0 ||
        equip->get_slot() < 0) {
        write<signed char>(2); // add movement
    }
}
