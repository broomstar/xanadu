...
{
	std::string x(kGameFilePath + n.name() + ".wz_reader");
	...
	// determine the wz_reader files version if it hasn't been done yet
	...
	if (wzmain->wz_version_ == 0)
	{
		...
		// try to read the contents of the wz_reader file using the encryption key
