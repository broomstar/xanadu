...
void Player::handle_chat_command() {
    ...
    switch (type) {
        case chat_command_handler_type_constants::kFind: {
            if (!receiver_player ||
                receiver_player->get_is_gm() && !get_is_gm()) {
                {
            ...
            if (receiver_player->get_channel_id() == channel_id_) {
                if (receiver_player->get_is_in_cash_shop()) {
                    {
                        PacketCreator packet;
                        packet.FindPlayer(receiver_player->get_name(),
                                          find_player_packet_mode2_constants::kCashshop,
                                          -1);
                        send_packet(&packet);
                    }
                } else {
                    {
                        PacketCreator packet;
                        packet.FindPlayer(receiver_player->get_name(),
                                          find_player_packet_mode2_constants::kMap,
                                          receiver_player->get_map()->get_id());
                        send_packet(&packet);
                    }
                }
            } else {
                {
                    PacketCreator packet;
                    packet.FindPlayer(receiver_player->get_name(),
                                      find_player_packet_mode2_constants::kChannel,
                                      receiver_player->get_channel_id());
                    send_packet(&packet);
                }
            }
            break;
...
void Player::handle_use_chat() {
    ...
    if (get_is_gm() && message[0] == '!') {
        ...
        if (command == "warp") {
            ...
            Player *target_player = world->GetPlayerByName(
        ...
        } else if (command == "pos") {
            {
                ...
                PacketCreator packet;
                packet.ShowMessage(
                        "x: " + std::to_string(position_x_) + " y: " +
                        std::to_string(position_y_) + " foothold: " +
                        std::to_string(foothold_) + " map: " +
                        std::to_string(map_->get_id()), 5);
                send_packet(&packet);
            }
        }
...

            {
                ...
                PacketCreator packet;
                packet.SpawnMist(object_id, id_, skill_id, position_x_,
                                 position_y_, mist_position_width,
                                 mist_position_height, skill_level);
                send_packet(&packet);
            }
        }
        ...
        else if (command == "posj") {
            {
                // packet
                PacketCreator packet;
                packet.leave_carnival_pq(false, mcpq_constants::kTeamBlue,
                                         name_);
                send_packet(&packet);
            }
        }
            ...
            ShopData *gmshop = ShopDataProvider::get_instance()->get_shop_data(
                    shopid);
            if (gmshop) {
                shop_ = gmshop;
            ...
            for (int i = 0; i < amount; ++i) {
                map_->spawn_monster(mob_id, position_x_, position_y_, 0,
                                    mob_constants::kSpawnTypesNewSpawn);
            }
...

            {
                ...
                PacketCreator packet;
                packet.ShowMessage(
                        "There are " + std::to_string(online_amount) +
                        " players online.", 5);
                send_packet(&packet);
            }
        ...
        if (command == "gm") {
            ...
            if (ticks < (last_gm_call_ticks_ + (3 * 60 * 1000))) {
                {
                    ...
                    PacketCreator packet;
                    packet.ShowMessage(
                            "Contacting the staff has a time limit of 3 minutes.",
                            1);
                    send_packet(&packet);
                }
                ...
                if (player->get_is_gm()) {
                    {
                        PacketCreator packet;
                        packet.ShowMessage(
                                "[HELP CALL] player: " + name_ + " message: " +
                                text, 6);
...

            {
                PacketCreator packet;
                packet.ShowMessage(
                        "A staff member will get to you as soon as possible.",
                        0);
                send_packet(&packet);
            }
        } else if (command == "fm") {
        ...
        } else if (command == "check") {
            {
                PacketCreator packet;
                packet.ShowMessage(
                        "NX Cash: " + std::to_string(nx_cash_credit_), 5);
                send_packet(&packet);
            }
        } else if (command == "str") {
