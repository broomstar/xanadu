...
void Player::handle_use_attack(signed char attack_type) {
    ...
    for (signed char i = 0; i < targets; ++i) {
        ...
        if (total_damage > 50000 && !get_is_gm() && level_ < 120) {
            std::string text =
                    "[WARNING] player: " + name_ + " made damage : " +
                    std::to_string(total_damage) + " with skill : " +
                    std::to_string(attack.skill_id_);
    ...
    if (star_slot > 0) {
        ...
        std::shared_ptr<Item> item = inventory->get_item_by_slot(
                static_cast<signed char>(star_slot));

        if (item) {
            attack.item_id_ = item->get_item_id();

            if (!is_buff_stat_active(buffstat_constants::kSoulArrow) &&
                // Soul Arrow
                !is_buff_id_active(4111009)) // Shadow Stars
            {
                inventory->remove_item_by_slot(
                        static_cast<signed char>(star_slot), hits);
            }
        }
    }
    ...
    if (is_buff_stat_active(buffstat_constants::kCombo) && targets > 0) {
        ...
        if (attack.skill_id_ == 1111003 || attack.skill_id_ == 1111005) {
            ...
            vals->push_back(
                    Value(buffstat_constants::kCombo, crusader_combo_value_));
            {
                PacketCreator packet;
