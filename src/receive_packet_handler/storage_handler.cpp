    ...
    switch (action) {
        case StorageReceivePacketActions::kTakeOutItem: {
...

            {
                ...
                packet.StoreOrTakeOutStorage(false, storage_slots_,
                                             inventory_id, storage_items_);
                send_packet(&packet);
            }
        ...
        case StorageReceivePacketActions::kStoreItem: {
            ...
            auto item = inventory->get_item_by_slot(
...

            {
                ...
                PacketCreator packet;
                packet.StoreOrTakeOutStorage(true, storage_slots_, inventory_id,
                                             storage_items_);
                send_packet(&packet);
            }
...
void Player::handle_merchant_storage_request() {
    ...
    switch (action) {
        case kFredrickHandlerActionTakeOut: {
            ...
            signed char free_equip_slots = get_inventory(
                    kInventoryConstantsTypesEquip)->get_free_slots();
            signed char free_etc_slots = get_inventory(
                    kInventoryConstantsTypesEtc)->get_free_slots();
            signed char free_use_slots = get_inventory(
                    kInventoryConstantsTypesUse)->get_free_slots();
            signed char free_setup_slots = get_inventory(
                    kInventoryConstantsTypesSetup)->get_free_slots();
            ...
            if (free_equip_slots < 0 || free_etc_slots < 0 ||
                free_use_slots < 0 || free_setup_slots < 0) {
                {
                    PacketCreator packet;
