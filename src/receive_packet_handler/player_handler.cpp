...
void Player::handle_player_hit() {
    ...
    if (is_buff_stat_active(buffstat_constants::kMagicGuard)) {
        int mp_damage = static_cast<int>(
                (damage * get_buff_value(buffstat_constants::kMagicGuard)) /
                100.0);
...
void Player::handle_change_map() {
    ...
    } else if (type == -1) // normal portal
    {
        ...
        Map *tomap = World::get_instance()->GetChannel(channel_id_)->get_map(
                portal->get_to_map_id());
        if (!tomap) {
            return;
        }

        if (portal->get_type() == 5 &&
            !map_->is_portal_enabled(portal->get_from_protal())) {
            {
                PacketCreator packet;
        ...
        MapData *data2 = MapDataProvider::get_instance()->get_map_data_by_id(
                tomap->get_id());
...
void Player::handle_get_player_info() {
    ...
    Player *target_player = World::get_instance()->GetPlayerById(
            target_player_id);
    {
        PacketCreator packet;
        if (target_player) {
