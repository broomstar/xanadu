...
#include "constants/party_constants.hpp"
#include "world.hpp"
#include "map.hpp"
...
void Player::handle_party_action() {
    ...
    switch (action) {
            ...
            if (party_->get_leader() == id_) {
                {
                    ...
                    packet.LeaveParty(party_, id_, false, std::string(""),
                                      false);
        ...
        case PartyReceivePacketActions::kExpel: {
            ...
            {
                PacketCreator packet;
                packet.LeaveParty(party_, target_player_id, true,
                                  member->get_player_name(), true);
                party_->send_packet(&packet);
            }
            ...
            Player *expelled = World::get_instance()->GetPlayerById(
                    target_player_id);
            if (expelled) {
