...
void Player::handle_cancel_skill_buff() {
    ...
    if (skill_id == mount_skill_id_ && mount_skill_id_ != 0) {
        Values vals;
        vals.get_values()->push_back(
                Value(buffstat_constants_position_1::kMonsterRiding, 0));
        {
            PacketCreator packet;
...
void Player::handle_use_skill() {
    ...
    SkillData *skill_data = SkillDataProvider::get_instance()->get_skill_data(
            skill_id);
    if (!skill_data) {
        return;
    }
    ...
    if (tools::skill_id_is_summon(skill_id)) {
        add_summon(skill_id, skill_level, skill_level_data->x,
                   skill_level_data->time);
        {
            PacketCreator packet;
            packet.EnableAction();
            send_packet(&packet);
                ...
                for (auto &it : *players) {
                    ...
                    if (target_player->get_map() == map_
                        && target_player->get_position_x() >
                           (position_x_ + skill_level_data->ltx)
                        && target_player->get_position_y() >
                           (position_y_ + skill_level_data->lty)
                        && target_player->get_position_x() <
                           (position_x_ + skill_level_data->rbx)
                        && target_player->get_position_y() <
                           (position_y_ + skill_level_data->rby)) {
