...
void Player::handle_use_pet() {
    ...
    std::shared_ptr<Item> pet = get_inventory(
            kInventoryConstantsTypesCash)->get_item_by_slot(slot);
...
void Player::handle_pet_movement() {
    ...
    if (pet_position_x != 0 || pet_position_y != 0 || pet_foothold != 0 ||
        pet_stance != 0) {
    ...
    if (map_->get_players()->size() > 1) {
        ...
        {
            PacketCreator packet;
            packet.MovePet(id_, pet->get_pet_slot(), start_position_x,
                           start_position_y,
