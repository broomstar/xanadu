...
void Player::handle_item_transportation() {
    ...
    switch (action) {
        case 0: // Create
        {
            ...
            switch (creation_type) {
                ...
                case 5: // Hired Merchant
                {
                    ...
                    if (merchant_storage_items_.size() > 0) {
                        {
                            PacketCreator packet;
                            packet.ShowMessage(
                                    "Please retrieve your stored items and mesos from NPC Fredrick in the Free Market Entrance first.",
                                    1);
                            send_packet(&packet);
                        }
                        return;
                    ...
                    auto store = inventory->get_item_by_slot(
                    ...
                    merchant_.reset(
                            new HiredMerchant(this, item_id, description));
                    {
                        PacketCreator packet;
        ...
        case 2: // Send trade request
        {
            ...
            Player *invited = World::get_instance()->GetPlayerById(
                    target_player_id);
        ...
        case 0x0A: // Exit Miniroom
        {
            if (trade_partner_) {
                if (trade_partner_->get_trade_partner() == this) {
                    ...
                    trade_partner_->add_mesos(
                            trade_partner_->get_trade_mesos());
        ...
        case 0x0F: // Add item to trade
        {
            ...
            if (!trade_partner_ ||
                trade_partner_->get_trade_partner() != this ||
                trade_partner_->get_map() != map_) {
                return;
            }
            ...
            std::shared_ptr<Item> item = inventory->get_item_by_slot(
                    static_cast<signed char>(item_slot));
            if (!item) {
                return;
            }
            ...
            inventory->remove_item_by_slot(static_cast<signed char>(item_slot),
                                           item_amount, true, true);
        ...
        case 0x11: // confirm the trade
        {
            ...
            for (size_t i = 0; i < trade_items_.size(); ++i) {
                ...
                trade_partner_->get_inventory(
                        item->get_inventory_id())->add_item_find_slot(item);
            }
        ...
        case 0x21: // Add item into Merchant
        {
            ...
            if ((item->get_amount() < (bundles * amount_per_bundle)) &&
                !item->is_star()) {
                return;
            }
            if (price < 1) {
            ...
            inventory->remove_item_by_slot(static_cast<signed char>(item_slot),
                                           (item->is_star() ? item->get_amount()
                                                            : (bundles *
                                                               amount_per_bundle)),
                                           true, true);

            std::shared_ptr<HiredMerchantItem> hiredmerchant_item(
                    new HiredMerchantItem(copy, bundles, amount_per_bundle,
                                          item_slot, price));
        ...
        case 0x22: // Buy item in Merchant
        {
            ...
            if (merchant_ && !merchant_->is_owner(id_) && amount > 0 &&
                merchant_->get_item(slot)) {
            ...
            if (merchant_ && merchant_->is_owner(id_)) {
                ...
                for (auto it : *items) {
                    ...
                    if (merchant_item->get_bundles() > 0) {
                        ...
                        merchant_storage_items_[counter]->set_amount(
                                merchant_item->get_quantity());
                        ++counter;
                    }
                }
                {
