...
void Player::handle_login_request() {
    ...
    // username check
    statement1 << "SELECT id FROM users WHERE username = '" << user_name_
               << "'", Poco::Data::Keywords::into(user_id_);
    ...
    statement2 << "SELECT id FROM users WHERE username = '" << user_name_
    ...
    statement3 << "SELECT id FROM users WHERE username = '" << user_name_
               << "' AND password = '" << password
               << "'", Poco::Data::Keywords::into(user_id_);
    statement3.execute();
    ...
    statement4 << "SELECT character_slots FROM users WHERE username = '"
               << user_name_ << "'", Poco::Data::Keywords::into(
            character_slots_);
    statement4.execute();
    ...
    statement5 << "SELECT id FROM characters WHERE user_id = "
               << user_id_, Poco::Data::Keywords::into(ids);
    statement5.execute();
    ...
    for (int id : ids) {
        ...
        statement6
                << "SELECT name, level, job, str, dex, intt, luk, fame, gender, skin, face, hair FROM characters WHERE id = "
                << id;
        statement6.execute();
        ...
        statement7 << "SELECT equip_id, pos FROM equips WHERE player_id = "
                   << id << " AND pos < 0";
...
void Player::handle_create_character() {
    ...
    if (!ValidCharDataProvider::get_instance()->is_valid_item(top_or_overall,
                                                              ValidItemType::Top)) {
        return;
    }

    int bottom = read<int>();
    if (!ValidCharDataProvider::get_instance()->is_valid_item(bottom,
                                                              ValidItemType::Bottom)) {
        return;
    }

    int shoes = read<int>();
    if (!ValidCharDataProvider::get_instance()->is_valid_item(shoes,
                                                              ValidItemType::Shoes)) {
        return;
    }

    int weapon = read<int>();
    if (!ValidCharDataProvider::get_instance()->is_valid_item(weapon,
                                                              ValidItemType::Weapon)) {
        return;
    }
    ...
    statement1
            << "INSERT INTO characters(name, user_id, job, str, dex, intt, luk, face, hair, skin, gender) VALUES("
            << "'" << name << "',"
            << user_id_ << ","
            << job_id << ","
            << str << ","
    ...
    int id = 0;
    Poco::Data::Statement statement2(mysql_session);
    statement2 << "SELECT LAST_INSERT_ID()", Poco::Data::Keywords::into(
            id), Poco::Data::Keywords::now;
    ...
    statement3 << "INSERT INTO keymap(player_id, pos, type, action) VALUES("
               << id << ", 18,4,0)" <<
               ",(" << id << ", 65,6,106)" <<
               ",(" << id << ", 2,4,10)" <<
               ",(" << id << ", 23,4,1)" <<
    ...
    Poco::Data::Statement statement4(mysql_session);
    statement4
            << "INSERT INTO equips(equip_id, player_id, weapon_attack, weapon_def, slots, pos) VALUES("
            << top_or_overall << "," << id << "," << "0, 3, 7,"
            << -kCreateCharacterEquipSlotsTop << ")", Poco::Data::Keywords::now;
    ...
    statement5
            << "INSERT INTO equips(equip_id, player_id, weapon_attack, weapon_def, slots, pos) VALUES("
            << bottom << "," << id << "," << "0, 2, 7,"
            << -kCreateCharacterEquipSlotsBottom
    ...
    statement6
            << "INSERT INTO equips(equip_id, player_id, weapon_attack, weapon_def, slots, pos) VALUES("
            << shoes << "," << id << "," << "0, 2, 5,"
            << -kCreateCharacterEquipSlotsShoes
            << ")", Poco::Data::Keywords::now;
    ...
    statement7
            << "INSERT INTO equips(equip_id, player_id, weapon_attack, weapon_def, slots, pos) VALUES("
            << weapon << "," << id << "," << "15, 0, 7,"
            << -kCreateCharacterEquipSlotsWeapon
            << ")", Poco::Data::Keywords::now;
...
void Player::handle_delete_character() {
    ...
    statement9 << "DELETE FROM merchant_storage_items WHERE player_id = "
               << player_id;
    statement9.execute();

    Poco::Data::Statement statement10(mysql_session);
    statement10 << "DELETE FROM merchant_storage_equips WHERE player_id = "
                << player_id;
    statement10.execute();
