...
void Player::handle_player_movement() {
    ...
    movement_handler(player_position_x, player_position_y, player_foothold,
                     player_stance);
    ...
    if (map_players->size() >= 2) {
        ...
        }
        {
            ...
            packet.ShowPlayerMovement(id_, start_position_x, start_position_y,
                                      session_->get_receive_buffer() +
                                      excluded_bytes, size);
