...
void Player::handle_mob_movement() {
    ...
    // send a packet to the player which is used to make the client continue mob movement for the player
    {
        ...
        packet.MoveMobResponse(mob, move_id, next_movement_could_be_skill,
    ...
    if (map_players->size() >= 2) {
        ...
        {
            PacketCreator packet;
            packet.MoveMob(monster_object_id, next_movement_could_be_skill,
                           action, skill_id, skill_level, option,
