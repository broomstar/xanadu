...
void Player::handle_player_drop_mesos() {
    ...
    std::shared_ptr<Drop> drop(new Drop(map_->get_drop_object_id(),
                                        kDropPacketConstantsPickupTypesFreeForAll,
                                        position_x_, player_position_y,
                                        dropped_mesos, 0, nullptr));
    map_->add_drop(drop);
    {
