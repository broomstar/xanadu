...
void Player::handle_use_item_effect() {
    int item_id = read<int>();
    if (item_id != 0) {
        signed char inventory_id = tools::get_inventory_id_from_item_id(
                item_id);
...
void Player::handle_use_cash_item() {
    ...
    std::shared_ptr<Item> item(cash_inventory->get_item_by_slot(
            static_cast<signed char>(item_slot)));
    if (!item) {
        return;
    }
    ...
    switch (item_id) {
        ...
        case 5390002: // LoveholicMessenger
        {
            ...
            {
                PacketCreator packet;
                packet.ShowAvatarMega(this, whisper, item_id, msg, msg2, msg3,
                                      msg4);
        ...
        case 5570000: // Vicious Hammer
        {
            ...
            std::shared_ptr<Item> equip = inventory->get_item_by_slot(
                    equip_slot);
            if (!equip) {
                return;
            }
...
void Player::handle_move_item() {
    ...
    if (is_drop) {
        ...
        if (amount != item1->get_amount()) {
...

            {
                ...
                packet.MoveItem(
                        (inventory_id1 == kInventoryConstantsTypesEquipped
                         || inventory_id2 == kInventoryConstantsTypesEquipped)
                        ? kInventoryConstantsTypesEquip : inventory_id1, slot1,
                        slot2);
                send_packet(&packet);
            }
        ...
        if (!item2) {
            ...
            {
                PacketCreator packet;
                packet.MoveItem((inventory_id1 == 0 || inventory_id2 == 0) ? 1
                                                                           : inventory_id1,
                                slot1, slot2);
                send_packet(&packet);
            }
            ...
            if (item1->get_amount() + item2->get_amount() <= max_per_slot) {
                ...
                {
                    PacketCreator packet;
                    packet.MoveItemMerge(inventory_id2, slot1, slot2,
                                         item2->get_amount());
                    send_packet(&packet);
                }
                inventory1->remove_item(slot1, true, false);
                ...
                {
                    PacketCreator packet;
                    packet.MoveItemMergeTwo(inventory_id1, slot1,
                                            item1->get_amount(), slot2,
                                            item2->get_amount());
                    send_packet(&packet);
                }
            }
...
void Player::handle_use_return_scroll() {
    ...
    auto item_data = ItemDataProvider::get_instance()->get_item_data(
            item->get_item_id());
...
void Player::handle_use_scroll() {
    ...
    Inventory *equipped_inventory = get_inventory(
    ...
    ItemData *item_data = ItemDataProvider::get_instance()->get_item_data(
            scroll->get_item_id());
    if (!item_data) {
        return;
    }
        ...
        switch (scroll->get_item_id()) {
            case 2049100: // Chaos Scroll 60%
            {
                ...
                if (equip->get_weapon_attack() > 0) {
                    equip->set_weapon_attack(equip->get_weapon_attack() +
                                             tools::random_int(-5, 5));
                }
                if (equip->get_magic_attack() > 0) {
                    equip->set_magic_attack(equip->get_magic_attack() +
                                            tools::random_int(-5, 5));
                }
                if (equip->get_weapon_defense() > 0) {
                    equip->set_weapon_defense(equip->get_weapon_defense() +
                                              tools::random_int(-5, 5));
                }
                if (equip->get_magic_defense() > 0) {
                    equip->set_magic_defense(equip->get_magic_defense() +
                                             tools::random_int(-5, 5));
                }
                if (equip->get_acc() > 0) {
                ...
                if (equip->get_avoid() > 0) {
                    equip->set_avoid(
                            equip->get_avoid() + tools::random_int(-5, 5));
                }
                if (equip->get_hand() > 0) {
                    equip->set_hand(
                            equip->get_hand() + tools::random_int(-5, 5));
                }
                if (equip->get_jump() > 0) {
                    equip->set_jump(
                            equip->get_jump() + tools::random_int(-5, 5));
                }
                if (equip->get_speed() > 0) {
                    equip->set_speed(
                            equip->get_speed() + tools::random_int(-5, 5));
                }
            ...
            case 2049137: // Incredible Chaos Scroll of Goodness 40%
            {
                ...
                if (equip->get_avoid() > 0) {
                    equip->set_avoid(
                            equip->get_avoid() + tools::random_int(1, 5));
                }
                if (equip->get_hand() > 0) {
                    equip->set_hand(
                            equip->get_hand() + tools::random_int(1, 5));
                }
                if (equip->get_jump() > 0) {
                    equip->set_jump(
                            equip->get_jump() + tools::random_int(1, 5));
                }
                if (equip->get_speed() > 0) {
                    equip->set_speed(
                            equip->get_speed() + tools::random_int(1, 5));
                }
            ...
            case 2049134: // Miraculous Chaos Scroll 70%
            {
                if (equip->get_str() > 0) {
                    equip->set_str(
                            equip->get_str() + tools::random_int(-10, 10));
                }
                if (equip->get_dex() > 0) {
                    equip->set_dex(
                            equip->get_dex() + tools::random_int(-10, 10));
                }
                if (equip->get_int() > 0) {
                    equip->set_int(
                            equip->get_int() + tools::random_int(-10, 10));
                }
                if (equip->get_luk() > 0) {
                    equip->set_luk(
                            equip->get_luk() + tools::random_int(-10, 10));
                }
                if (equip->get_hp() > 0) {
                ...
                if (equip->get_acc() > 0) {
                    equip->set_acc(
                            equip->get_acc() + tools::random_int(-10, 10));
                }
                if (equip->get_avoid() > 0) {
                    equip->set_avoid(
                            equip->get_avoid() + tools::random_int(-10, 10));
                }
                if (equip->get_hand() > 0) {
                    equip->set_hand(
                            equip->get_hand() + tools::random_int(-10, 10));
                }
                if (equip->get_jump() > 0) {
                    equip->set_jump(
                            equip->get_jump() + tools::random_int(-10, 10));
                }
                if (equip->get_speed() > 0) {
                    equip->set_speed(
                            equip->get_speed() + tools::random_int(-10, 10));
                }
            ...
            default: // normal scrolls
            {
                ...
                equip->set_weapon_attack(
                        equip->get_weapon_attack() + item_data->watk);
                equip->set_magic_attack(
                        equip->get_magic_attack() + item_data->matk);
                equip->set_weapon_defense(
                        equip->get_weapon_defense() + item_data->wdef);
                equip->set_magic_defense(
                        equip->get_magic_defense() + item_data->mdef);
    ...
    get_inventory(kInventoryConstantsTypesUse)->remove_item_by_slot(scroll_slot,
                                                                    1, false);

    if (cursed) {
        get_inventory(kInventoryConstantsTypesEquipped)->remove_item_by_slot(
                equip_slot, 1, false);
    }
}
...
void Player::handle_inventory_sort() {
    ...
    for (signed char slot2 = 1;
         slot2 <= static_cast<signed char>(items.size()); ++slot2) {
