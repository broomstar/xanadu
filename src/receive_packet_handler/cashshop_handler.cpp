...
#include "constants/cashshop_constants.hpp"
...
void Player::handle_cash_shop_enter() {
    ...
    {
        PacketCreator packet;
        packet.GetCashShopInventory(cashshop_storage_items_, user_id_,
                                    storage_slots_, character_slots_);
        send_packet(&packet);
    }
    {
        PacketCreator packet;
...
void Player::handle_cash_shop_action() {
    ...
    switch (action) {
        case CashShopReceivePacketActions::kBuyCashItem: {
            ...
            CashItemData *cash_item = CashShopItemDataProvider::get_instance()->get_cash_item_data(
                    serial_number);
            if (!cash_item) {
                return;
            }
        ...
        case CashShopReceivePacketActions::kBuyInventorySlots: {
            ...
            {
                PacketCreator packet;
                packet.IncreaseInventorySlots(inventory_id,
                                              inventory->get_slots());
                send_packet(&packet);
            }
            break;
        ...
        case CashShopReceivePacketActions::kStoreCashItem: {
            ...
            Inventory *equip_inventory = get_inventory(
            ...
            for (auto it : *equip_inventory->get_items()) {
                ...
                equip_inventory->remove_item_by_slot(target_item_slot, 1,
                                                     false);
            ...
            if (!worked) {
                Inventory *cash_inventory = get_inventory(
                ...
                for (auto it : *cash_inventory->get_items()) {
                    ...
                    signed char target_item_slot = it.second->get_slot();
                    cash_inventory->remove_item_by_slot(target_item_slot, 1,
                                                        false);
        ...
        case CashShopReceivePacketActions::kBuyPackage: {
            ...
            CashItemData *package_cash_item = CashShopItemDataProvider::get_instance()->get_cash_item_data(
            ...
            std::vector<int> serial_numbers = CashShopPackageDataProvider::get_instance()->get_serial_numbers_in_cash_package(
                    package_id);
