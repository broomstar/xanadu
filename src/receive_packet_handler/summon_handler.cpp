...
void Player::handle_summon_movement() {
    ...
    movement_handler(summon_position_x, summon_position_y, summon_foothold,
                     summon_stance);

    if (summon_position_x != 0 || summon_position_y != 0 ||
        summon_foothold != 0 || summon_stance != 0) {
    ...
    if (map_->get_players()->size() > 1) {
        ...
        }
        {
            ...
            packet.MoveSummon(id_, start_position_x, start_position_y,
                              session_->get_receive_buffer() + excluded_bytes,
                              recv_length_ - excluded_bytes);
