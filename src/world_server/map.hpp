...
#include "constants/mob_constants.hpp"
...
public:
    ...
    void drop_mesos_from_mob(short position_x, short position_y, Mob *mob,
                             int amount, int owner);

    void drop_item_from_mob(short position_x, short position_y, Mob *mob,
                            std::shared_ptr<Item> item, int owner);

    void drop_from_player(std::shared_ptr<Item> item, Player *player);

    void loot_drop(Player *player, std::shared_ptr<Drop> drop,
                   signed char pet_slot = -1);

    void kill(Mob *mob);

    void spawn_monster(int mob_id, short x, short y, short foothold = 0,
                       signed char spawn_type = mob_constants::kSpawnTypesNewSpawn,
                       bool flip = false, int from = -1);

    void
    spawn_monster_init(int mob_id, short x, short y, short foothold, bool flip);
