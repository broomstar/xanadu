    ...
    for (signed char i = 1; i <= slots_; ++i) {
        if (items_.find(i) == items_.end() || !items_[i]) {
            ...
            if (items_[i]->get_item_id() == item->get_item_id()
                ...
                if (item->get_amount() + items_[i]->get_amount() <=
                    max_per_slot) {
                    items_[i]->set_amount(
                            items_[i]->get_amount() + item->get_amount());
...
void Inventory::remove_item_by_slot(signed char slot, short amount, bool send,
                                    bool trading) {
    std::shared_ptr<Item> item = get_item_by_slot(slot);
    if (!item) {
        return;
    ...
    if (item->get_amount() < 1) {
        ...
        if (!item->is_star() ||
            trading) // stars are removed from inventory only in trade
        {
