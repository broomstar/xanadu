...
#include "constants/drop_constants.hpp"
#include "constants/server_constants.hpp"
#include "constants/pq_constants.hpp"
...
void Map::cleanup_timer_callback(const std::error_code &ec) {
    ...
    cleanup_timer_.async_wait(std::bind(&Map::cleanup_timer_callback, this,
                                        std::placeholders::_1));
}
...
void Map::add_player(Player *player) {
    ...
    */
    ...
    for (auto it : drops_) {
        ...
        {
            PacketCreator packet;
            packet.ShowDrop(kDropPacketConstantsAnimationTypesShowExisting,
                            drop, 0, 0);
            player->send_packet(&packet);
        }
    }
    ...
    for (auto target_player : players_) {
        ...
        if (target_player->get_summon()
            && (!target_player->is_hidden() || player->get_is_gm())) {
            {
                // send a packet
                PacketCreator packet;
                packet.SpawnSummon(target_player, target_player->get_summon(),
                                   false);
                player->send_packet(&packet);
            }
        }
    ...
    for (auto pet : *pets) {
        pet->set_position(player->get_position_x(), player->get_position_y(),
                          player->get_stance());
        {
            // packet
            PacketCreator packet;
    ...
    if (!cleanup_timer_running_) {
        ...
        std::error_code ec;
        cleanup_timer_.expires_from_now(std::chrono::seconds(timer_seconds),
                                        ec);
        cleanup_timer_.async_wait(std::bind(&Map::cleanup_timer_callback, this,
                                            std::placeholders::_1));
    }
...
signed char Map::get_closest_spawn_point(short position_x, short position_y) {
    ...
    for (auto it : *portals) {
        ...
        int dis = tools::get_distance(portal->get_x(), portal->get_y(),
...
void Map::find_foothold(short position_x, short &position_y) {
    ...
    for (MapFootholdData *fh : *footholds) {
        if ((x > fh->x1 && x <= fh->x2) || (x > fh->x2 && x <= fh->x1)) {
            short cmax = static_cast<short>(static_cast<float>(
                    (fh->y1 - fh->y2) / (fh->x1 - fh->x2) * (x - fh->x1) +
                    fh->y1));

            if ((cmax <= maxy || (maxy == position_y && firstcheck)) &&
                cmax >= y) {
                maxy = cmax;
                firstcheck = false;
            }
...
bool Map::can_open_store(Player *player) {
    ...
    for (auto it : hired_merchants_) {
        ...
        if (merchant != player->get_hired_merchant()) {
            if (tools::get_distance(merchant->get_position_x(),
                                    merchant->get_position_y(),
                                    player->get_position_x(),
                                    player->get_position_y()) <= 23000) {
                {
                    // packet
                    PacketCreator packet;
...
std::unordered_map<int, std::shared_ptr<HiredMerchant>> *
Map::get_hired_merchants() {
...
void Map::loot_drop(Player *player, std::shared_ptr<Drop> drop,
...
void Map::drop_from_mob(Mob *mob, int killer_player_id) {
    ...
    if (tools::random_int(1, 100) <= 20) {
        drop_pos_x = (mob->get_position_x() +
                      25 * ((count + count % 2) / 2) * ((count % 2) ? 1 : -1));
    ...
    auto drops = MobDropsDataProvider::get_instance()->get_drop_data(
            mob->get_monster_id());
    ...
    for (auto it : *drops) {
        ...
        if (random_value < drop_chance) {
            ...
            drop_item_from_mob(drop_pos_x, drop_pos_y, mob, item,
                               killer_player_id);
        }
    }
}

void Map::drop_item_from_mob(short position_x, short position_y, Mob *mob,
                             std::shared_ptr<Item> item, int owner) {
    find_foothold(position_x, position_y);
    std::shared_ptr<Drop> drop(new Drop(get_drop_object_id(),
                                        kDropPacketConstantsPickupTypesNormal,
                                        position_x, position_y, 0, owner,
                                        item));

    drops_[drop->get_id()] = drop;
    {
        PacketCreator packet;
        packet.ShowDrop(kDropPacketConstantsAnimationTypesDropAnimation, drop,
                        mob->get_position_x(), mob->get_position_y());
        send_packet(&packet);
    }
}

void Map::drop_mesos_from_mob(short position_x, short position_y, Mob *mob,
                              int amount, int owner) {
    find_foothold(position_x, position_y);
    std::shared_ptr<Drop> drop(new Drop(get_drop_object_id(),
                                        kDropPacketConstantsPickupTypesNormal,
                                        position_x, position_y, amount, owner,
                                        nullptr));
...
void Map::drop_from_player(std::shared_ptr<Item> item, Player *player) {
    ...
    std::shared_ptr<Drop> drop(
            new Drop(drop_id, kDropPacketConstantsPickupTypesFreeForAll, x, y,
                     0, 0, item));
...
void Map::kill(Mob *mob) {
    ...
    MobData *data = MobDataProvider::get_instance()->get_data_by_id(
            mob->get_monster_id());
    ...
    for (auto monster_id : data->mob_summons_) {
        spawn_monster(monster_id, mob->get_position_x(), mob->get_position_y(),
                      mob->get_original_foothold(),
                      mob_constants::kSpawnTypesSummonByMob,
                      mob->get_stance() % 2 == 0, mob->get_object_id());
    }
    ...
    if (bossing_) {
        // zakum
        if (mob->get_monster_id() >= 8800003 &&
            mob->get_monster_id() <= 8800010) {
            Mob *zakum_body = nullptr;

            for (Mob *temp_mob : mobs_) {
                if (!temp_mob->is_dead()) {
                    if (temp_mob->get_monster_id() >= 8800003 &&
                        temp_mob->get_monster_id() <= 8800010) {
                        return;
                    }
...
void Map::spawn_monster(int mob_id, short x, short y, short foothold,
                        signed char spawn_type, bool flip, int from) {
    ...
    Mob *mob = new Mob(this, mob_id, get_mob_object_id(), x, y, foothold,
                       flip ? 3 : 2, data->max_hp_, data->max_mp_,
                       data->is_boss_, false);
...
void Map::spawn_monster_init(int mob_id, short x, short y, short foothold,
                             bool flip) {
    MobData *data = MobDataProvider::get_instance()->get_data_by_id(mob_id);
    if (!data) {
        return;
