...
#include "timer/timer.hpp"
#include "summon.hpp"
#include "constants/buffstat_constants.hpp"
...
Effect *Effect::get_skill_effect(int id, signed char level) {
    if (skill_effects_.find(id) != skill_effects_.end() &&
        skill_effects_[id].find(level) != skill_effects_[id].end()) {
...
Effect::Effect(int id, SkillLevelData *data) :
    ...
    switch (id) {
        case 2001002: // Mage Magic Guard
            values->push_back(
                    Value(buffstat_constants::kMagicGuard, skill_data_->x));
            break;
        case 2301003: // Cleric Invincible
            values->push_back(
                    Value(buffstat_constants::kInvincible, skill_data_->x));
            break;
        case 2121004: // FPArchMage Infinity
        case 2221004: // ILArchMage Infinity
        case 2321004: // Bishop Infinity
            values->push_back(
                    Value(buffstat_constants::kInfinityBuff, skill_data_->x));
            values->push_back(
                    Value(buffstat_constants::kStance, skill_data_->prop));
            break;
        case 2311003: // Priest Holy Symbol
        case 9101002: // SuperGM Holy Symbol
            values->push_back(
                    Value(buffstat_constants::kHolySymbol, skill_data_->x));
            break;
        case 4201011: // ChiefBandit Meso Guard
            values->push_back(
                    Value(buffstat_constants::kMesoGuard, skill_data_->x));
            break;
        case 4211003: // ChiefBandit Pickpocket
            values->push_back(
                    Value(buffstat_constants::kPickPocket, skill_data_->x));
            break;
        case 4111002: // Hermit Shadow Partner
        case 4211008: // Chief Bandit Shadow Partner
            values->push_back(
                    Value(buffstat_constants::kShadowPartner, skill_data_->x));
            break;
        case 4001003: // Thief Darksight
            values->push_back(
                    Value(buffstat_constants::kDarksight, skill_data_->x));
            break;
        ...
        case 5201003: // Gunslinger Gun Booster
            values->push_back(
                    Value(buffstat_constants::kBooster, skill_data_->x));
            break;
        case 1121000:
        case 1221000:
        case 1321000:
        ...
        case 5221000: // Pirate Maple Warrior
            values->push_back(
                    Value(buffstat_constants::kMapleWarrior, skill_data_->x));
            break;
        ...
        case 1221004:  // Holy Charge
            values->push_back(
                    Value(buffstat_constants::kWkCharge, skill_data_->x));
            break;
        ...
        case 3201004:
            values->push_back(
                    Value(buffstat_constants::kSoulArrow, skill_data_->x));
            break;
        case 3121002:  // Bow Master Sharp Eyes
        case 3221002:  // Marksman Sharp Eyes
            values->push_back(Value(buffstat_constants::kSharpEyes,
                                    (skill_data_->x << 8 | skill_data_->y)));
            break;
        case 3111000: // Concentrate
        case 3211000:
            values->push_back(
                    Value(buffstat_constants::kConcentrate, skill_data_->x));
            break;
        case 1111002: // Combo
...
void
handle_stop_cooldown(int player_id, int skill_id, const std::error_code &ec) {
    if (ec) {
        return;
    }
...
void Effect::use_attack_skill(Player *player) {
    ...
    if (skill_data_->cooldown_time > 0) {
        ...
        timer->get_object()->async_wait(
                std::bind(&handle_stop_cooldown, player->get_id(), id_,
                          std::placeholders::_1));
        auto timers = player->get_timers();
...
void Effect::use_skill(Player *player) {
    ...
    if (id_ == 1121010) {
        ...
        player->set_crusader_combo_value(
                player->get_crusader_combo_value() - 10);

        Values m;
        auto vals = m.get_values();
        vals->push_back(Value(buffstat_constants::kCombo,
                              player->get_crusader_combo_value()));
        {
            PacketCreator packet;
    ...
    if (skill_data_->required_item_id > 0) {
        player->remove_item(skill_data_->required_item_id,
                            skill_data_->required_item_count);
    }
    ...
    if (skill_data_->ltx != 0 && skill_data_->rbx != 0 && player->get_party()) {
        ...
        for (auto it : *players) {
            ...
            if (target_player != player &&
                target_player->get_map() == player->get_map()) {
                if (target_player->get_position_x() >
                    player->get_position_x() + skill_data_->ltx &&
                    target_player->get_position_y() >
                    player->get_position_y() + skill_data_->lty &&
                    target_player->get_position_x() <
                    player->get_position_x() + skill_data_->rbx &&
                    target_player->get_position_y() <
                    player->get_position_y() + skill_data_->rby) {
                    if (target_player->get_hp() > 0) {
                        {
                            PacketCreator packet;
                            packet.ShowBuffEffect(target_player->get_id(), 2,
                                                  id_, level_);
                            target_player->get_map()->send_packet(&packet,
                                                                  target_player);
                        }
                        {
                            PacketCreator packet;
                        ...
                        if (id_ == 1301007 ||
                            id_ == 9101008) // Hyper Body - SuperGM Hyper Body
                        {
                            int max_hp_value = (target_player->get_max_hp() /
                                                100 * skill_data_->x);
                            int max_mp_value = (target_player->get_max_mp() /
                                                100 * skill_data_->x);
                            target_player->set_hyperbody_max_hp(max_hp_value);
                            target_player->set_hyperbody_max_mp(max_mp_value);
                        }

                        bool is_buff = (skill_data_->time > 0 &&
                                        values_.get_values()->size() > 0);
                        if (is_buff) {
                            target_player->add_buff(this);

                            {
                                PacketCreator packet;
                                packet.ShowPlayerBuff(&values_, id_,
                                                      skill_data_->time);
                                target_player->send_packet(&packet);
                            }
                            // timer
                            timer::Id timer_id(timer::kTimerTypesBuffTimer,
                                               id_);
                            std::shared_ptr<Timer> timer(
                                    new Timer(skill_data_->time / 1000));
                            timer->get_object()->async_wait(
                                    std::bind(&handle_stop_buff,
                                              target_player->get_id(), id_,
                                              std::placeholders::_1));
    ...
    if (tools::is_monster_riding(id_)) {
        ...
        if (id_ == 1004) // 1004 is the monster rider skill id
        {
            if (player->get_inventory(
                    kInventoryConstantsTypesEquipped)->get_item_by_slot(
                    kItemConstantsEquippedSlotsTamingMobSaddle)
                && player->get_inventory(
                    kInventoryConstantsTypesEquipped)->get_item_by_slot(
                    kItemConstantsEquippedSlotsTamingMob)) {
                mount_id = player->get_inventory(
                        kInventoryConstantsTypesEquipped)->get_item_by_slot(
                        kItemConstantsEquippedSlotsTamingMob)->get_item_id();
            }
        }
    ...
    if (is_buff) {
        ...
        timer->get_object()->async_wait(
                std::bind(&handle_stop_buff, player->get_id(), id_,
                          std::placeholders::_1));
        auto timers = player->get_timers();
        timers->operator[](timer_id) = timer;
        ...
        if (values_.get_value_by_stat(buffstat_constants::kShadowPartner) !=
            0) {
