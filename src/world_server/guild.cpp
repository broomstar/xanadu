...
void Guild::SetLogoColor(signed char color_, bool init) {
    ...
    if (!init) {
        ...
        statement << "UPDATE guilds SET logo_color = "
                  << static_cast<int>(logo_color_)
                  << " WHERE guild_id = " << id_;
        statement.execute();
    }
}
...
void Guild::SetLogoBackgroundColor(signed char background_color, bool init) {
    ...
    if (!init) {
        ...
        Poco::Data::Session &mysql_session = World::get_instance()->get_mysql_session();
        Poco::Data::Statement statement(mysql_session);
        statement << "UPDATE guilds SET logo_bg_color = "
                  << static_cast<int>(logo_background_color_)
                  << " WHERE guild_id = " << id_;
        statement.execute();
    }
}
...
void Guild::SetLogoBackground(short background, bool init) {
    ...
    if (!init) {
        ...
        Poco::Data::Session &mysql_session = World::get_instance()->get_mysql_session();
        Poco::Data::Statement statement(mysql_session);
        statement << "UPDATE guilds SET logo_bg = "
                  << static_cast<int>(logo_background_)
                  << " WHERE guild_id = " << id_;
        statement.execute();
    }
}
...
void Guild::AddMember(Player *player) {
    ...
    members_[player_id].reset(
            new GuildMember(player_id, player->get_job(), player->get_level(),
                            player->get_guild_rank(), 1, player->get_name()));
}
