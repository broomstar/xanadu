...
Item::Item(int item_id, bool random_equip_stats) :
        ...
        hammers_used_(0) {
    if (is_pet()) {
        PetData *data = PetDataProvider::get_instance()->get_data_by_id(
                item_id);
        if (data) {
...
bool Item::is_pet() {
    bool check = PetDataProvider::get_instance()->get_data_by_id(item_id_) !=
                 nullptr;
    return check;
}

bool Item::is_equip() {
    bool check = EquipDataProvider::get_instance()->get_item_data(item_id_) !=
                 nullptr;
    return check;
}
