...
#include "constants/receive_packet_opcodes.hpp"
#include "channel.hpp"
#include "world.hpp"
#include "summon.hpp"
#include "quest.hpp"
...
#include "connection_handling/session.hpp"
        ...
        if (trade_partner_) {
            if (trade_partner_->get_trade_partner() == this) {
                ...
                for (auto item : *trade_partner_items) {
                    Inventory *inventory = trade_partner_->get_inventory(
                            item->get_inventory_id());
                    if (inventory) {
                        inventory->add_item_find_slot(item);
                    }
                }
...
void Player::player_connect() {
    ...
    statement1 << "SELECT user_id FROM characters WHERE id = "
               << id_, Poco::Data::Keywords::into(user_id_);
    ...
    statement2 << "SELECT username FROM users WHERE id = "
               << user_id_, Poco::Data::Keywords::into(user_name_);
    ...
    inventories_[kInventoryConstantsTypesEquipped] = new Inventory(this,
                                                                   kInventoryConstantsTypesEquipped,
                                                                   48);
    inventories_[kInventoryConstantsTypesEquip] = new Inventory(this,
                                                                kInventoryConstantsTypesEquip,
                                                                equip_slots);
    inventories_[kInventoryConstantsTypesUse] = new Inventory(this,
                                                              kInventoryConstantsTypesUse,
                                                              use_slots);
    inventories_[kInventoryConstantsTypesSetup] = new Inventory(this,
                                                                kInventoryConstantsTypesSetup,
                                                                setup_slots);
    inventories_[kInventoryConstantsTypesEtc] = new Inventory(this,
                                                              kInventoryConstantsTypesEtc,
                                                              etc_slots);
    inventories_[kInventoryConstantsTypesCash] = new Inventory(this,
                                                               kInventoryConstantsTypesCash,
                                                               36);

    {
        ...
        statement
                << "SELECT gm, character_slots, storage_mesos, nxcash_credit FROM users WHERE id = "
                << user_id_;
        statement.execute();
...

    {
        // storage equips
        Poco::Data::Statement statement(mysql_session);
        statement << "SELECT * FROM storage_equips WHERE user_id = "
                  << user_id_;
        ...
        for (; cols > 0; --cols) {
            ...
            EquipData *data = EquipDataProvider::get_instance()->get_item_data(
                    equip->get_item_id());
            ...
            if (data) {
                ...
                for (i = 0; i < storage_items_.size(); ++i) {
                    if (storage_items_[i]->get_inventory_id() >
                        equip->get_inventory_id()) {
                        break;
                    }
                }
...

    {
        ...
        Poco::Data::Statement statement(mysql_session);
        statement
                << "SELECT item_id, pos, amount, flags FROM storage_items WHERE user_id = "
                << user_id_;
        size_t cols = statement.execute();
        ...
        for (; cols > 0; --cols) {
            ...
            ItemData *data = ItemDataProvider::get_instance()->get_item_data(
                    item->get_item_id());
...

    {
        ...
        Poco::Data::Statement statement(mysql_session);
        statement
                << "SELECT item_id, pos, amount FROM merchant_storage_items WHERE player_id = "
                << id_;
        size_t cols = statement.execute();
...

    {
        ...
        Poco::Data::Statement statement(mysql_session);
        statement << "SELECT * FROM merchant_storage_equips WHERE player_id = "
                  << id_;
        size_t cols = statement.execute();
...

    {
        ...
        for (; cols > 0; --cols) {
            ...
            signed char inventory_id = (equip->get_slot() > 0
                                        ? kInventoryConstantsTypesEquip
                                        : kInventoryConstantsTypesEquipped);
...

    {
        ...
        Poco::Data::Statement statement(mysql_session);
        statement
                << "SELECT item_id, pos, amount, flags FROM items WHERE player_id = "
                << id_;
        size_t cols = statement.execute();
...

    {
        ...
        Poco::Data::Statement statement(mysql_session);
        statement
                << "SELECT pet_id, pos, pet_slot, level, closeness FROM pets WHERE player_id = "
                << id_;
        size_t cols = statement.execute();
        ...
        for (; cols > 0; --cols) {
            ...
            if (bool check =
                    PetDataProvider::get_instance()->get_data_by_id(pet_id) !=
                    nullptr)
                //if (inventory_constants::is_pet(pet_id))
            {
                ...
                get_inventory(
                        kInventoryConstantsTypesCash)->add_item_no_find_slot(
                        pet, false);
...

    {
        ...
        for (; cols > 0; --cols) {
            ...
            QuestData *data = QuestDataProvider::get_instance()->get_quest_data(
                    quest_id);

            if (data) {
                initialize_player_quests(quest_id, is_completed, mob_id,
                                         amount);
            }
...

    {
        ...
        Poco::Data::Statement statement(mysql_session);
        statement
                << "SELECT skill_id, level, master_level FROM skills WHERE player_id = "
                << id_;
        size_t cols = statement.execute();
        ...
        for (; cols > 0; --cols) {
            ...
            SkillData *data = SkillDataProvider::get_instance()->get_skill_data(
                    skill_id);
    ...
    {
        Poco::Data::Statement statement(mysql_session);
        statement << "SELECT pos, type, action FROM keymap WHERE player_id = "
                  << id_;
        size_t cols = statement.execute();
...

    {
        // load buddylist
        Poco::Data::Statement statement(mysql_session);
        statement << "SELECT buddy_id FROM buddy_lists WHERE player_id = "
                  << id_;
        size_t cols = statement.execute();
        ...
        for (; cols > 0; --cols) {
            ...
            if (buddy && buddy->get_buddy(id_)) {
                add_buddy(buddy->get_id(), buddy->get_name(),
                          buddy->get_channel_id());
                change_buddy_channel(buddy, id_, channel_id_);
            } else {
                std::string buddy_name = world->get_player_name_from_id(
                        buddy_id);
...
bool Player::add_mesos(int amount) {
    if (amount < 0) {
        ...
        if (mesos_test >
            INT_MAX) // prevent players having more than INT_MAX (~2,1 billion usually) mesos
        {
            return false;
        }
    }
...
void Player::level_up() {
    ...
    if (level_ == 200 && get_is_gm() == false) {
        {
            PacketCreator packet;
            packet.ShowMessage("[Congrats] " + name_ +
                               " has reached Level 200! Congratulate " + name_ +
                               " on such an amazing achievement!", 6);
...
short Player::get_item_amount(int itemid) {
    ...
    Inventory *inventory = get_inventory(
            tools::get_inventory_id_from_item_id(itemid));
    if (!inventory) {
        return amount;
    }
...
bool Player::give_item(int item_id, short amount) {
    Inventory *inventory = get_inventory(
            tools::get_inventory_id_from_item_id(item_id));
    if (!inventory) {
        return false;
    }
...
void Player::save_character() {
    ...
    statement1 << "UPDATE characters SET level = '" << static_cast<int>(level_)
               ...
               << "', map = '" << map_->get_id()
               << "', pos = '"
               << static_cast<int>(map_->get_closest_spawn_point(position_x_,
                                                                 position_y_))
               ...
               << "', mesos = '" << mesos_
               << "', party_id = '"
               << static_cast<int>((party_) ? party_->get_id() : 0)
               << "', guild_id = '"
               << static_cast<int>((guild_) ? guild_->get_id() : 0)
               << "', guild_rank = '" << static_cast<int>(guild_rank_)
               << "', equip_slots = '" << static_cast<int>(get_inventory(
            kInventoryConstantsTypesEquip)->get_slots())
               << "', use_slots = '" << static_cast<int>(get_inventory(
            kInventoryConstantsTypesUse)->get_slots())
               << "', setup_slots = '" << static_cast<int>(get_inventory(
            kInventoryConstantsTypesSetup)->get_slots())
               << "', etc_slots = '" << static_cast<int>(get_inventory(
            kInventoryConstantsTypesEtc)->get_slots())
...
void Player::save_skills() {
    ...
    for (auto &it : skills_) {
        if (firstrun) {
            statement2
                    << "INSERT INTO skills (player_id, skill_id, level, master_level) VALUES (";
            firstrun = false;
        } else {
            statement2 << ",(";
        }
        statement2 << id_ << "," << it.first << ","
                   << static_cast<short>(it.second.level_) << ","
                   << static_cast<short>(it.second.master_level_) << ")";
    }
    if (!firstrun) {
        statement2.execute();
    }
...
void Player::save_equips() {
    ...
    for (int i = kInventoryConstantsTypesEquipped;
         i <= kInventoryConstantsTypesEquip; ++i) {
        Poco::Data::Statement statement2(mysql_session);
        bool firstrun = true;
        ...
        for (auto &it : *items) {
            ...
            if (firstrun) {
                statement2
                        << "INSERT INTO equips (equip_id, player_id, pos, slots, used_scrolls, str, dex, iint, luk, hp, mp, weapon_attack, magic_attack, weapon_def, magic_def, accuracy, avoid, hand, speed, jump, flags) VALUES";
                firstrun = false;
            } else {
                statement2 << ",";
            }
...
void Player::save_items() {
    ...
    for (int i = kInventoryConstantsTypesUse;
         i <= kInventoryConstantsTypesCash; ++i) {
        Poco::Data::Statement statement2(mysql_session);
        bool firstrun = true;
        auto items = get_inventory(i)->get_items();
        ...
        for (auto &it : *items) {
            ...
            if (!item->is_pet()) {
                if (firstrun) {
                    statement2
                            << "INSERT INTO items (item_id, player_id, pos, amount, flags) VALUES";
                    firstrun = false;
                } else {
                    statement2 << ",";
                }
                statement2 << "(" << item->get_item_id() << "," << id_ << ","
...
void Player::save_pets() {
    ...
    for (auto &it : *items) {
        ...
        if (pet->is_pet()) {
            if (firstrun) {
                statement2
                        << "INSERT INTO pets (pet_id, player_id, pos, pet_slot, level, closeness) VALUES";
                firstrun = false;
            } else {
                statement2 << ",";
            }
            statement2 << "(" << pet->get_item_id() << "," << id_ << ","
                       << static_cast<int>(pet->get_slot()) << ","
                       << static_cast<int>(pet->get_pet_slot()) << ","
                       << static_cast<int>(pet->get_pet_level()) << ","
                       << static_cast<int>(pet->get_pet_closeness()) << ")";
        }
    }
...
void Player::save_keymap() {
    ...
    for (int pos = kMinKeymapPos; pos < kMaxKeymapPos; ++pos) {
        ...
        if (firstrun) {
            statement2
                    << "INSERT INTO keymap (player_id, pos, type, action) VALUES";
            firstrun = false;
        } else {
            statement2 << ",";
        }
        statement2 << "(" << id_ << "," << pos << ","
                   << static_cast<int>(key.type) << "," << key.action << ")";
    }
    if (!firstrun) {
        statement2.execute();
    }
...
void Player::save_quests() {
    ...
    for (auto &it : completed_quests_) {
        ...
        if (firstrun) {
            statement2
                    << "INSERT INTO quests (player_id, quest_id, is_complete) VALUES";
            firstrun = false;
        } else {
            statement2 << ",";
        }
    ...
    for (auto &it : quests_in_progress_) {
        ...
        if (quest) {
            if (firstrun) {
                statement3
                        << "INSERT INTO quests (player_id, quest_id, is_complete, killed_mob, amount) VALUES";
                firstrun = false;
            } else {
                statement3 << ",";
            }
...
void Player::save_buddylist() {
    ...
    for (auto &it : buddies_) {
        ...
        if (firstrun) {
            statement2
                    << "INSERT INTO buddy_lists (player_id, buddy_id) VALUES";
            firstrun = false;
        } else {
            statement2 << ",";
        }
...
void Player::save_storage_equips() {
    ...
    for (auto equip : storage_items_) {
        ...
        if (firstrun) {
            statement2
                    << "INSERT INTO storage_equips (equip_id, user_id, pos, slots, used_scrolls, str, dex, iint, luk, hp, mp, weapon_attack, magic_attack, weapon_def, magic_def, accuracy, avoid, hand, speed, jump, flags) VALUES";
            firstrun = false;
        } else {
            statement2 << ",";
        }
...
void Player::save_storage_items() {
    ...
    for (auto item : storage_items_) {
        if (item->get_inventory_id() == kInventoryConstantsTypesEquip ||
            item->get_inventory_id() == kInventoryConstantsTypesCash) {
            continue;
        }
        if (firstrun) {
            statement2
                    << "INSERT INTO storage_items (item_id, user_id, pos, amount, flags) VALUES";
            firstrun = false;
        } else {
            statement2 << ",";
        }
...
void Player::save_merchant_storage_equips() {
    ...
    statement1 << "DELETE FROM merchant_storage_equips WHERE player_id = "
               << id_;
    statement1.execute();
    ...
    for (auto it : merchant_storage_items_) {
        ...
        if (firstrun) {
            statement2
                    << "INSERT INTO merchant_storage_equips (equip_id, player_id, pos, slots, used_scrolls, str, dex, iint, luk, hp, mp, weapon_attack, magic_attack, weapon_def, magic_def, accuracy, avoid, hand, speed, jump, flags) VALUES";
            firstrun = false;
        } else {
            statement2 << ",";
        }
...
void Player::save_merchant_storage_items() {
    ...
    statement1 << "DELETE FROM merchant_storage_items WHERE player_id = "
               << id_;
    statement1.execute();
    ...
    for (auto it : merchant_storage_items_) {
        ...
        if (firstrun) {
            statement2
                    << "INSERT INTO merchant_storage_items (item_id, player_id, pos, amount, flags) VALUES";
            firstrun = false;
        } else {
            statement2 << ",";
        }
...
std::unordered_map<signed char, std::shared_ptr<Item>> *
Player::get_merchant_storage_items() {
...
void Player::send_npc() {
    ...
    ShopData *npc_shop = ShopDataProvider::get_instance()->get_shop_data(
            shop_id);
    if (npc_shop) {
        shop_ = npc_shop;
        {
    ...
    switch (npc_->id_) {
        ...
        case 9030000: // Fredrick
        {
            ...
            packet20.GetFredrickStorage(merchant_storage_items_,
                                        merchant_storage_mesos_);
        ...
        case 9030100: // Scrooge
        {
            ...
            packet22.GetStorage(npc_->id_, storage_slots_, storage_items_,
                                storage_mesos_);
