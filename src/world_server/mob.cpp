...
Mob::Mob(Map *map, int monster_id, int object_id, short position_x,
         short position_y, short foothold, signed char stance, int max_hp,
         int max_mp, bool is_boss, bool is_map_mob) :
        map_(map),
        is_dead_(false),
...
void Mob::take_damage(Player *player, int damage) {
    ...
    if (data->is_boss_) {
        ...
        if (color != 0) {
            ...
            packet28.ShowBossHp(monster_id_, hp_, data->max_hp_, color,
                                data->hp_background_color_);
            map_->send_packet(&packet28);
        }
    } else {
        signed char hp_percentage = static_cast<signed char>(hp_ * 100.0 /
                                                             data->max_hp_);
...
void Mob::give_exp() {
    ...
    for (auto &it : playersa_) {
        ...
        if (p) {
            ...
            if (!party) {
                ...
                int exp = static_cast<int>(lround(it.second * kExpRate * 100.0 *
                                                  MobDataProvider::get_instance()->get_data_by_id(
                                                          monster_id_)->exp_ /
                                                  100.0 /
                                                  MobDataProvider::get_instance()->get_data_by_id(
                                                          monster_id_)->max_hp_));
                ...
                if (p->get_inventory(
                        kInventoryConstantsTypesCash)->get_item_amount(
                        5211000) > 0) {
                    exp *= 2;
                }
    ...
    for (auto &d : damages) {
        ...
        for (auto &it : *ps) {
            if (it.second->get_map() == map_) {
                ...
                if (playersa_.find(it.second->get_id()) != playersa_.end()
                    || it.second->get_level() >=
                       maxplayer[party->get_id()]->get_level() - 5
                    || it.second->get_level() >=
                       MobDataProvider::get_instance()->get_data_by_id(
                               monster_id_)->level_ - 5) {
        ...
        int baseexp = static_cast<int>(lround(d.second * kExpRate * 100.0 *
                                              MobDataProvider::get_instance()->get_data_by_id(
                                                      monster_id_)->exp_ /
                                              100.0 /
                                              MobDataProvider::get_instance()->get_data_by_id(
        ...
        if (count == 1) {
            ...
            maxplayer[party->get_id()]->set_exp(
                    maxplayer[party->get_id()]->get_exp() + baseexp);
        ...
        for (auto &player : players) {
            if (player->get_hp() > 0) {
                int exp = static_cast<int>(lround(
                        baseexp * 0.8 * player->get_level() / levels));
                ...
                if (exp > 0) {
                    ...
                    // holy symbol
                    if (player->is_buff_stat_active(
                            buffstat_constants::kHolySymbol)) {
                    ...
                    packet20.GainExp(exp, false,
                                     (player == maxplayer[party->get_id()]),
                                     bonus);
...
void Mob::find_control(bool spawn) {
    ...
    for (Player *player : *players) {
        int distance = tools::get_distance(position_x_, position_y_,
    ...
    signed char spawn_type = (spawn ? mob_constants::kSpawnTypesNewSpawn
                                    : mob_constants::kSpawnTypesOldSpawn);
