...
HiredMerchant::HiredMerchant(Player *player, int item_id,
...
std::unordered_map<int, std::shared_ptr<HiredMerchantItem>> *
HiredMerchant::get_items() {
    return &items_;
}
...
void HiredMerchant::return_item(short slot, Player *owner) {
    ...
    if (sellItem && sellItem->get_bundles() > 0) {
        ...
        if (!item->is_star()) {
            item->set_amount(
                    sellItem->get_bundles() * sellItem->get_quantity());
        }

        slot = sellItem->get_slot();
        std::shared_ptr<Item> olditem = owner->get_inventory(
                item->get_inventory_id())->get_item_by_slot(
                static_cast<signed char>(slot));
        ...
        if (olditem) {
            ...
            if (item->is_star()
                ...
                if ((item->get_amount() + olditem->get_amount()) >
                    item_data->max_per_slot) {
                    item->set_amount(item->get_amount() -
                                     (item_data->max_per_slot -
                                      olditem->get_amount()));
                    ...
                    item->set_amount(
                            item->get_amount() + olditem->get_amount());
                    owner->get_inventory(
                            item->get_inventory_id())->remove_item_by_slot(
                            static_cast<signed char>(slot), 1);
