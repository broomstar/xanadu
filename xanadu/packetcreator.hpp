...
public:
    ...
    void LoginRequest(signed char success_or_failure_reason, int user_id,
    ...
    void ShowAllCharacterInfo(int world_id,
    ...
    void ShowCharacters(std::unordered_map<int, Character *> *characters,
                        int character_slots);
    ...
    void GetCashShopInventory(std::vector<std::shared_ptr<Item>> items,
                              int account_id, short storage_slots,
                              short character_slots);
    ...
    void TransferToCashShopInventory(const std::shared_ptr<Item> &item,
                                     int account_id);

    void CashShopShowBoughtPackage(std::vector<std::shared_ptr<Item>> items,
                                   int account_id);

    void
    CashShopAddCashItemData(const std::shared_ptr<Item> &item, int account_id);

    // chat
    void ShowChatMessage(Player *player, const std::string &message,
                         bool bubble_only);

    void ShowMessage(const std::string &message, unsigned char type,
                     unsigned char channel_id = 0, unsigned char whisper = 0,
                     std::shared_ptr<Item> item = nullptr);

    void FindPlayerReply(const std::string &name, bool success);

    void FindPlayer(const std::string &name, signed char mode2,
                    int map_id_or_channel_id);

    void WhisperPlayer(Player *player, const std::string &message);

    void ShowGroupChat(unsigned char type, const std::string &name,
                       const std::string &message);

    // drops
    void ShowDrop(signed char type, std::shared_ptr<Drop> drop, short pos_x,
                  short pos_y);

    void RemoveDrop(int drop_id, signed char animation, int player_id,
                    signed char pet_slot);
    ...
    void GuildPlayerLeave(int guild_id, int char_id, std::string char_name,
                          bool expelled = false);
    ...
    void AddGuildToAlliance(/*MapleAlliance alliance, */int new_guild,
                                                        Player *player);
    ...
    void RemoveGuildFromAlliance(/*MapleAlliance alliance, */int expelled_guild,
                                                             Player *player);
    ...
    void MoveItem(signed char inventory_id, short source_slot,
    ...
    void MoveItemMerge(signed char inventory_id, short source_slot,
                       short destination_slot, short amount);

    void MoveItemMergeTwo(signed char inventory_id, short source_slot,
                          short source_amount, short destination_slot,
                          short destination_amount);

    void ScrolledItem(std::shared_ptr<Item> scroll, std::shared_ptr<Item> equip,
                      bool destroyed);
    ...
    void ShowAvatarMega(Player *player, unsigned char ear, int item_id,
                        std::string message, std::string message2,
    ...
    void
    UseScroll(int player_id, bool success, bool cursed, bool legendary_spirit);
    ...
    void
    MovePet(int owner_player_id, signed char pet_slot, short start_position_x,
            short start_position_y, unsigned char *buffer, int size);

    void
    ShowPetChat(int owner_player_id, std::shared_ptr<Item> pet, signed char act,
                const std::string &message);

    void PetCommandReplay(int owner_player_id, std::shared_ptr<Item> pet,
                          signed char animation);
    ...
    void MoveMob(int mob_object_id, bool use_skill, signed char action,
                 signed char skill_id, signed char skill_level, short option,
                 short start_position_x, short start_position_y,
                 unsigned char *buffer, int buffer_size);

    void MoveMobResponse(Mob *mob, short move_id, bool use_skill,
                         signed char skill_id, signed char skill_level);

    void ShowBossHp(int mob_id, int hp, int max_hp, signed char color,
                    signed char background_color);
    ...
    void DamageSummon(int player_id, int skill_id, int damage, signed char unk,
                      int mob_id_from);

    void
    DamageMobSummon(Player *player, int summon_object_id, signed char animation,
    ...
    void GetFredrickStorage(
            std::unordered_map<signed char, std::shared_ptr<Item>> items,
            int mesos);

    void GetStorage(int npc_id, signed char slots,
                    std::vector<std::shared_ptr<Item>> items, int mesos);

    void GetStorageFull();

    void MesoStorage(signed char slots, int mesos);

    void StoreOrTakeOutStorage(bool store, signed char slots,
                               signed char inventory_id,
    ...
    void ShowPlayerMovement(int player_id, short start_position_x,
                            short start_position_y, unsigned char *buffer,
                            int buffer_size);
    ...
    void ShowSkillEffect(int player_id, int skill_id, signed char skill_level,
                         signed char flags, signed char speed);
    ...
    void ShowBuffEffect(int player_id, signed char effect_id, int skill_id,
                        signed char skill_level);
    ...
    void
    SpawnDoor(int object_id, bool town, short position_x, short position_y);

    void RemoveDoor(int object_id, bool town);

    void SpawnMist(int object_id, int owner_character_id, int skill_id,
                   int mist_position_x, int mist_position_y,
                   int mist_position_width, int mist_position_height,
                   int skill_level);
    ...
    void
    GetHiredMerchant(Player *player, std::shared_ptr<HiredMerchant> merchant,
                     bool first_time);
    ...
    void MoveSummon(int owner_player_id, short start_position_x,
                    short start_position_y, unsigned char *buffer, int size);
    ...
    void carnival_pq_summon(signed char tab, signed char summon_number,
                            std::string player_name);

    void carnival_pq_died(signed char lost_cp, unsigned char team,
                          std::string player_name);

    void leave_carnival_pq(bool player_is_leader, unsigned char team,
                           std::string player_name);
