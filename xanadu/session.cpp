...
void Session::initialize() {
    ...
    asio::async_write(socket_,
                      ...
                      std::bind(&Session::send_handler,
                                ...
                                std::placeholders::_2, send_buffer));
...
void Session::send_packet(PacketCreator *packet) {
    ...
    crypto::create_packet_header(send_buffer, iv_send_,
                                 static_cast<unsigned short>(packet_length));

    // encrypt the packet
    crypto::encrypt(send_buffer + 4, iv_send_,
                    static_cast<unsigned short>(packet_length));
...
void Session::read_header_handler(const std::error_code &ec,
                                  std::size_t bytes_transferred) {
    if (!ec) {
        // get the packet length from the header buffer
        unsigned short packet_length = crypto::get_packet_length(
                receive_buffer_.get());
...
void Session::read_body_handler(const std::error_code &ec,
                                std::size_t bytes_transferred) {
    if (!ec) {
...
void
Session::send_handler(const std::error_code &ec, std::size_t bytes_transferred,
