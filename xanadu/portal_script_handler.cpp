...
void Player::handle_use_scripted_portal() {
    ...
    if (script_name == "hontale_morph") // Horntail entrance map
    {
        ...
        set_map(103020100);
               ...
               "Zakumgo") // The Cave of Trial III - The Door to Zakum
    {
        set_map(211042300);
    } else if (script_name ==
               "Zakum05") // The Door to Zakum - Entrance to Zakum Altar
    {
        set_map(211042400);
    } else if (script_name == "Populatus00") // Papulatus Entrance
    {
        ...
        if (file != nullptr) {
            char text[100] = "";
            sprintf(text, "scripted portal: %s missing in map: %d",
                    script_name.c_str(), map_->get_id());
            fprintf(file, "%s", text);
            fprintf(file, "\n");
            fclose(file);
        }
